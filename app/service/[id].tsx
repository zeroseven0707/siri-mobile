...
import { Search, X, Star, UtensilsCrossed, MapPin, ChevronRight, Bike, FileText, Info } from 'lucide-react-native';
...
export default function ServiceOrderScreen() {
  ...
  if (service.slug === 'food') {
    return (
      <View style={styles.container}>
        <Stack.Screen options={{ title: service.name, headerStyle: { backgroundColor: '#fff' }, headerTintColor: '#000', headerShadowVisible: false }} />
        <View style={styles.searchHeader}>
          <View style={styles.searchBar}>
            <Search size={18} color="#9CA3AF" />
            <TextInput style={styles.searchInput} placeholder="Cari restoran atau makanan..." value={query} onChangeText={setQuery} />
            {query.length > 0 && (
        ...
        {isSearching ? (
          <View style={styles.loadingContainer}><ActivityIndicator color={GREEN} /></View>
        ) : (
          <ScrollView contentContainerStyle={styles.storeList} showsVerticalScrollIndicator={false}>
            <Text style={styles.sectionTitle}>{query ? `Hasil pencarian "${query}"` : 'Restoran Populer'}</Text>
            {stores.map((store) => (
  ...
  if (!user) {
    return (
      <View style={styles.container}>
        ...
        <AuthPlaceholder icon="Bike" title={`Pesan ${service.name}`} description={`Silakan masuk untuk melakukan pemesanan ${service.name} dan menikmati seluruh fitur Siri.`} />
      </View>
    );
  }
  ...
  return (
    <>
      <Stack.Screen options={{ title: `Pesan ${service.name}`, headerStyle: { backgroundColor: '#fff' }, headerTintColor: '#1F2937', headerShadowVisible: false }} />
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={100}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>

          <View style={styles.serviceBanner}>
            <View style={styles.serviceIconCircle}>
              <Bike size={32} color={GREEN} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.serviceBannerTitle}>{service.name}</Text>
              <Text style={styles.serviceBannerSub}>{service.description || 'Cepat, aman, dan terpercaya'}</Text>
            </View>
          ...
          <View style={styles.sectionCard}>
            <View style={styles.sectionCardHeader}>
              ...
              <Text style={styles.sectionCardTitle}>{service.name.toLowerCase() === 'nitah' ? 'Mau nitah apa?' : 'Tujuan'}</Text>
            </View>
            <View style={styles.notesBox}>
              <View style={{ marginTop: 2 }}><MapPin size={18} color="#EF4444" /></View>
              <TextInput
                style={styles.notesInput}
                placeholder={service.name.toLowerCase() === 'nitah' ? 'Temenin ke dokter, Bantuin push rank, apa aja bisaa' : 'Ketik alamat tujuan lengkap\nContoh: Jl. Merdeka No. 10, Kel. Sukajadi, Kec. Bandung Wetan, Kota Bandung'}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8 }}>
              ...
              <Text style={[styles.destHint, { marginTop: 0, marginLeft: 4 }]}>
                {service.name.toLowerCase() === 'nitah' ? 'Jelaskan apa yang kamu butuhkan agar kami bisa membantu dengan tepat.' : 'Sertakan nama jalan, nomor, kelurahan, dan kota agar driver mudah menemukan lokasi kamu.'}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.sectionCard}>
            ...
            <View style={styles.notesBox}>
              <View style={{ marginTop: 2 }}><FileText size={18} color="#9CA3AF" /></View>
              <TextInput
                style={styles.notesInput}
          ...
          <Pressable style={[styles.btnOrder, isSubmitting && { opacity: 0.7 }]} onPress={handleOrder} disabled={isSubmitting}>
            {isSubmitting ? <ActivityIndicator color="#fff" /> : (
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <View style={{ marginRight: 8 }}><Bike size={18} color="#fff" /></View>
